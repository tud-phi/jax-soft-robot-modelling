        ...
        for i in range(nb_colomns):
            for j in range(nb_rows):
                ...
                for j_type in list_of_type_of_jacobian:
                    J = {"symbolic": J_symb, "explicit": J_exp, "autodiff": J_auto}[
                        j_type
                    ]
                    axs[i, j].plot(
                        kappa_values,
...
kappa_slider = Slider(
    plt.axes([0.2, 0.17, 0.65, 0.03]), "kappa max", 0.01, 0.5, valinit=borne_kappa
)
...
def update_sliders(_):
    on_slider_change(
        s_slider.val, sigma_x_slider.val, sigma_y_slider.val, kappa_slider.val
    )
