...
REORDERED_LINES_FWD_KINE = jnp.array(
    [1, 2, 0]
)  # reorder the lines to match the forward kinematics vector [x, y, theta] => [sigma_x, sigma_y, kappa_z]
REORDERED_LINES_STRAIN = jnp.array(
    [2, 0, 1]
)  # reorder the lines to match the strain vector [kappa_z, sigma_x, sigma_y] => [theta, x, y]
...
def factory(
    ...
    Array,
    Callable[[Dict[str, Array], Array, Array, float], Array],
    Callable[
    ...
    if not isinstance(num_segments, int):
        raise ValueError(
            f"num_segments must be an integer, but got {type(num_segments)}"
        )
    if num_segments < 1:
    ...
    if not isinstance(strain_selector, jnp.ndarray):
        if isinstance(strain_selector, list):
            ...
            raise TypeError(
                f"strain_selector must be a jnp.ndarray, but got {type(strain_selector).__name__}"
            )
    ...
    if not isinstance(xi_eq, jnp.ndarray):
        if isinstance(xi_eq, list):
            ...
            raise TypeError(
                f"xi_eq must be a jnp.ndarray, but got {type(xi_eq).__name__}"
            )
    xi_eq = xi_eq.flatten()
    ...
    if not callable(stiffness_fn):
        raise TypeError(
            f"stiffness_fn must be a callable, but got {type(stiffness_fn).__name__}"
        )
    ...
    if not callable(actuation_mapping_fn):
        raise TypeError(
            f"actuation_mapping_fn must be a callable, but got {type(actuation_mapping_fn).__name__}"
        )
    ...
    if integration_type == "gauss-legendre":
        ...
        raise ValueError(
            f"integration_type must be either 'gauss-legendre', 'gauss-kronrad' or 'trapezoid', but got {integration_type}"
        )

    if jacobian_type not in ["explicit", "autodiff"]:
        raise ValueError(
            f"jacobian_type must be either 'explicit' or 'autodiff', but got {jacobian_type}"
        )
    ...
    def apply_eps_to_bend_strains(xi: Array, eps: float) -> Array:
        """
        Add a small number to the bending strain to avoid singularities.

        Args:
            xi (Array): strain vector of the robot.
            eps (float): small number to add to the bending strain.

        Returns:
            Array: strain vector with the bending strain modified.
        """
        if eps == None:
            return xi
        else:
    ...
    def chi_fn(
        params: Dict[str, Array], xi: Array, s: Array, eps: float = global_eps
    ) -> Array:
        """
        ...
        Returns:
            chi (Array): pose of the robot at the point s in the interval [0, L]
                The pose is represented as a vector [x, y, theta] where (x, y) is the position
        ...
        # Iteration function
        def chi_i(i: int, chi_prev: Array) -> Array:
    ...
    def J_explicit_local(
        ...
        Returns:
            J_local (Array) : body-frame jacobian of the forward kinematics function with respect to the strain vector
        ...
        J_0_L0 = jnp.concatenate(
            [mat_0_L0[None, :, :], jnp.zeros((num_segments - 1, 3, 3))], axis=0
        )
        J_0_s = jnp.concatenate(
            [mat_0_s[None, :, :], jnp.zeros((num_segments - 1, 3, 3))], axis=0
        )
        ...
        _, J_array = lax.scan(f=J_i, init=tuple_J_0, xs=jnp.arange(1, num_segments))
        ...
        J_segment_SE2_local = lax.dynamic_index_in_dim(
            J_array, segment_idx, axis=0, keepdims=False
        )

        # Reorder the lines to match the forward kinematics function
        J_local = J_segment_SE2_local[
            :, REORDERED_LINES_FWD_KINE, :
        ]  # shape: (n_segments, 3, 3)
    ...
    def J_explicit_global(
        ...
        g_s = jnp.block([[R, jnp.zeros((2, 1))], [jnp.zeros((1, 2)), jnp.eye(1)]])
        ...
        J_segment_SE2_global = jnp.einsum(
            "ij,njk->nik", Adjoint_g_s, J_segment_SE2_local
        )  # shape: (n_segments, 6, 6)

        # Reorder the lines to match the forward kinematics function
        J_global = J_segment_SE2_global[
            :, REORDERED_LINES_FWD_KINE, :
        ]  # shape: (n_segments, 3, 3)
    ...
    def J_Jd_explicit_local(
        ...
        Returns:
            ...
            J_d_local (Array): body-frame time-derivative of the jacobian of the forward kinematics function.
        """
        # Classify the point along the robot to the corresponding segment
        segment_idx, _, l_cum = classify_segment(params, s)
        ...
        xi_d_SE2_i = vmap(lambda i: lax.dynamic_slice(xi_d, (3 * i,), (3,)))(
            idx_range
        )  # shape: (num_segments, 3)
        S_i = vmap(
            lambda i: lax.dynamic_index_in_dim(
                J_segment_SE2_local, i, axis=0, keepdims=False
            )
        )(idx_range)  # shape: (num_segments, 3, 3)
        sum_Sj_xi_d_j = compute_weighted_sums(
            J_segment_SE2_local, xi_d_SE2_i, num_segments
        )  # shape: (num_segments, 3)
        adjoint_sum = vmap(adjoint_SE2)(sum_Sj_xi_d_j)  # shape: (num_segments, 3, 3)

        # Compute the time-derivative of the Jacobian
        J_d_segment_SE2_local = jnp.einsum(
            "ijk, ikl->ijl", adjoint_sum, S_i
        ...
        J_d_local = J_d_segment_SE2_local[
            :, REORDERED_LINES_FWD_KINE, :
        ]  # shape: (n_segments, 3, 3)
    ...
    def J_Jd_explicit_global(
        ...
        Returns:
            ...
            J_d_global (Array): inertial-frame time-derivative of the jacobian of the forward kinematics function.
        """
        # Classify the point along the robot to the corresponding segment
        segment_idx, _, l_cum = classify_segment(params, s)
        ...
        J_d_segment_global = jnp.einsum(
            "ij,njk->nik", Adjoint_g_s, J_d_segment_SE2_local
        )  # shape: (n_segments, 6, 6)
    ...
    def jacobian_fn(
        ...
        """
        Compute the inertial-frame jacobian of the forward kinematics function with respect to the configuration vector q.
    ...
    def B_autodiff_fn(params: Dict[str, Array], xi: Array) -> Array:
        """
        ...
        def compute_integral(i):
            if integration_type == "gauss-legendre":
                Xs, Ws, nGauss = gauss_quadrature(
                    N_GQ=param_integration, a=l_cum[i], b=l_cum[i + 1]
                )
                ...
                integral, _, _, _ = rule.integrate(
                    integrand, l_cum[i], l_cum[i + 1], args=()
                )
    ...
    def C_autodiff_fn(
        params: Dict[str, Array], xi: Array, xi_d: Array
    ) -> Tuple[Array, Array]:
        """
        Compute the Coriolis / centrifugal matrix of the robot
        using autodiff.
        ...
        # # ===========================================
        # # Native for loop version
        # C = jnp.zeros((n_xi, n_xi))  # Initialize the Coriolis matrix
        # for i in range(n_xi):
        #     for j in range(n_xi):
        #         for k in range(n_xi):
        ...
        #             return acc + coeff
        #         C_ij = lax.fori_loop(0, n_xi, body_k, 0.0)
        #         return C.at[i, j].set(C_ij)
        #     return lax.fori_loop(0, n_xi, body_j, C)
        # C = lax.fori_loop(0, n_xi, body_i, C)

        # ===========================================
        # Vmap version
        def C_ij(i, j):
            cs_k = vmap(lambda k: christoffel_fn(i, j, k))(jnp.arange(n_xi))
            return jnp.dot(cs_k, xi_d)

        C = vmap(lambda i: vmap(lambda j: C_ij(i, j))(jnp.arange(n_xi)))(
            jnp.arange(n_xi)
        )

        # # ===========================================
        # # LAX map version
        # def C_ij(i, j, xi_d, n_xi):
        #     cs_k = lax.map(lambda k: christoffel_fn(i, j, k), jnp.arange(n_xi))
        #     return jnp.dot(cs_k, xi_d)
        # C = jnp.stack(
        #         jnp.stack(
        #             lax.map(
        #                 lambda i: lax.map(
        #                     lambda j: C_ij(i, j, xi_d, n_xi),
        #                     jnp.arange(n_xi)
        #                 ),
        #                 jnp.arange(n_xi))
        #             )
    ...
    def B_explicit_fn(params: Dict[str, Array], xi: Array) -> Array:
        """
    ...
    def C_explicit_fn(
        ...
        def compute_integral_C(i):
            if integration_type == "gauss-legendre":
                ...
                J_all, J_d_all = vmap(
                    lambda s: J_Jd_explicit_local(params, xi, xi_d, s)
                )(Xs)  # [[Jp1],[Jp2],[Jo]]
                J_all = J_all[:, REORDERED_LINES_STRAIN, :]  # [[Jo],[Jp1],[Jp2]]
                J_d_all = J_d_all[
                    :, REORDERED_LINES_STRAIN, :
                ]  # [[Jo_d],[Jp1_d],[Jp2_d]]
                M_a = rho[i] * jnp.diag(
                    jnp.stack([Ib[i], A[i], A[i]], axis=0)
                )  # [[O],[P],[P]]

                integrand_C = vmap(
                    lambda _J_i, _J_d_i: (
                        _J_i.T
                        @ (adjoint_star_SE2((_J_i @ xi_d)) @ M_a @ _J_i + M_a @ _J_d_i)
                    )
                )(J_all, J_d_all)
                ...
                integral_C, _, _, _ = rule.integrate(
                    integrand, l_cum[i], l_cum[i + 1], args=()
                )
    ...
    def U_g_fn_xi(
        params: Dict[str, Array], xi: Array, eps: float = global_eps
    ) -> Array:
        """
    ...
    if jacobian_type == "explicit":
        B_fn_xi = B_explicit_fn
        C_fn_xi = C_explicit_fn
        G_fn_xi = G_explicit_fn
    elif jacobian_type == "autodiff":
        B_fn_xi = B_autodiff_fn
        C_fn_xi = C_autodiff_fn
        G_fn_xi = G_autodiff_fn

    def dynamical_matrices_fn(
        params: Dict[str, Array], q: Array, q_d: Array, eps: float = global_eps
    ) -> Tuple[Array, Array, Array, Array, Array, Array]:
        """
    ...
    def kinetic_energy_fn(
        ...
        Returns:
            T (Array): kinetic energy of shape ()
        """
    ...
    def potential_energy_fn(
        ...
        Args:
            ...
            eps (float, optional): small number to avoid singularities (e.g., division by zero). Defaults to global_eps.
        Returns:
            U (Array): potential energy of shape ()
        """
        # Map the configuration to the strains
        xi = xi_eq + B_xi @ q
        # Add a small number to the bending strain to avoid singularities
    ...
    def operational_space_dynamical_matrices_fn(
        ...
        Args:
            ...
            q_d (Array): generalized velocities of shape (n_q, )
            s (Array): point coordinate along the robot in the interval [0, L].
            B (Array): mass / inertia matrix of the robot in the generalized coordinates of shape (n_q, n_q)
            C (Array): Coriolis / centrifugal matrix of the robot in the generalized coordinates of shape (n_q, n_q)
            operational_space_selector (Tuple, optional): tuple of shape (3,) to select the operational space variables.
                For example, (True, True, False) selects only the positional components of the operational space.
            eps (float, optional): small number to avoid singularities (e.g., division by zero). Defaults to global_eps.
        Returns:
            Lambda (Array): inertia matrix in the operational space of shape (3, 3)
            mu (Array): coriolis and centrifugal matrix in the operational space of shape (3, )
            J (Array): inertial-frame jacobian of the end-effector pose with respect to the generalized coordinates
            J_d (Array): inertial-frame time-derivative of the jacobian of the end-effector pose with respect to the generalized coordinates
                of shape (3, n_q)
            JB_pinv (Array): Dynamically-consistent pseudo-inverse of the inertial-frame jacobian. Allows the mapping of torques
