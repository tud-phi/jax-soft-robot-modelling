...
def factory(
    ...
    def dynamical_matrices_fn(
    ...
    ) -> Tuple[Array, Array, Array, Array, Array, Callable]:
        """
        ...
        Returns:
            ...
            alpha_fn: callable to compute the torque resulting from actuation. Signature: alpha_fn(phi) -> tau_q
        """
        # map the configuration to the strains
        xi = xi_eq + B_xi @ q
        xi_d = B_xi @ q_d
...
def ode_factory(
    ...
    def ode_fn(t: float, x: Array, *args) -> Array:
        ...
        B, C, G, K, D, alpha_fn = dynamical_matrices_fn(params, q, q_d)

        # inverse of B
        B_inv = jnp.linalg.inv(B)

        # compute the acceleration
        q_dd = B_inv @ (alpha_fn(phi) - C @ q_d - G - K - D @ q_d)
